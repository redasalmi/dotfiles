...
export function renderWindow(
  ...
  return `${ctx.ui.theme.fg("mdLink", `${label}:`)} ${ctx.ui.theme.fg("success", `${left}% left`)} ${ctx.ui.theme.fg("dim", `(${used}% used, ${formatRemainingTime(window)})`)}`;
}
...
export function renderIndividualLimit(ctx: ExtensionContext, limit: IndividualLimit): string | undefined {
  ...
  return `${ctx.ui.theme.fg("mdLink", "monthly:")} ${ctx.ui.theme.fg("success", `${left}% left`)} ${ctx.ui.theme.fg("dim", `(${formatRemainingTime(limit)}${usage})`)}`;
}
