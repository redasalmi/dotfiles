...
function mergeWindow(current: UsageWindow | undefined, update: UsageWindow | undefined): UsageWindow | undefined {
...
export function createUsage(pi: ExtensionAPI, state: CodexState, deps: UsageDeps) {
  ...
  async function refresh(ctx: ExtensionContext, force = false): Promise<boolean> {
    ...
    promise = (async () => {
      try {
        ...
        const previousResetCreditCount = state.resetCreditCount;
        const previousStatusStale = state.statusStale;
        state.snapshots = snapshotsFromUsage(usage);
        state.resetCreditCount = usage.rate_limit_reset_credits?.available_count;
        state.statusStale = false;
        if (!deps.renderStatus(ctx)) {
          state.snapshots = previousSnapshots;
          state.resetCreditCount = previousResetCreditCount;
          state.statusStale = previousStatusStale;
  ...
  async function showResetCredits(ctx: ExtensionContext): Promise<void> {
    ...
    let credits = state.snapshots.get("codex")?.resetCredits ?? [];
    ...
    options.push("Cancel");
    const selected = ctx.mode === "tui" ? await ctx.ui.select("Redeem usage limit reset", options) : options[0];
    ...
    if (ctx.mode === "tui" && !(await ctx.ui.confirm("Confirm usage limit reset", `Redeem ${detail}? This consumes one saved reset.`))) return;
    ...
    try {
      ...
      const retry = ctx.mode === "tui" && (await ctx.ui.confirm("Reset redemption failed", "Retry the same request safely?"));
      if (!retry) {
