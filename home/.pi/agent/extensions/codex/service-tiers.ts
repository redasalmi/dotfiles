...
function isFastCapableModel(modelId: string): boolean {
  return modelId === "gpt-5.4" || modelId === "gpt-5.5" || modelId === "gpt-5.6" || modelId.startsWith("gpt-5.6-");
}
...
function legacyServiceTiers(model: Model<any>): ServiceTier[] {
  // Older locally cached model catalogs predate service_tiers. Keep the
  // documented Fast models working until their catalog metadata is refreshed.
  return isFastCapableModel(model.id) ? [{ id: "priority", name: "Fast" }] : [];
}
...
export function parseServiceTiers(model: Model<any> | undefined): ServiceTier[] {
  ...
  // Once a model advertises tier metadata, including an empty list, trust it.
  // The legacy fallback is only for catalogs that have no tier metadata at all.
  return raw === undefined ? legacyServiceTiers(model) : normalizeTiers(raw);
}
